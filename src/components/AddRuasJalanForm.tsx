...
export const AddRuasJalanForm: React.FC<Props> = ({ onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (onSuccess) onSuccess(); // trigger close form
        } catch (error) {
        ...
        <div className="max-w-2xl mx-auto p-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <Input value={formData.kode_ruas} onChange={(e) => handleChange("kode_ruas", e.target.value)} required />
                </div>
                <div className="flex flex-col gap-1">
                    <Label>Nama Ruas</Label>
                    <Input value={formData.nama_ruas} onChange={(e) => handleChange("nama_ruas", e.target.value)} required />
                </div>
                <div className="flex flex-col gap-1">
                    <Label>Panjang</Label>
                    <Input value={formData.panjang} onChange={(e) => handleChange("panjang", e.target.value)} required />
                </div>
                <div className="flex flex-col gap-1">
                    <Label>Lebar</Label>
                    <Input value={formData.lebar} onChange={(e) => handleChange("lebar", e.target.value)} required />
                </div>
                <div className="flex flex-col gap-1">
                    <Label>Keterangan</Label>
                    <Input value={formData.keterangan} onChange={(e) => handleChange("keterangan", e.target.value)} />
                </div>
                <div className="flex flex-col gap-1">
                    <Label>Desa</Label>
                    <Select
